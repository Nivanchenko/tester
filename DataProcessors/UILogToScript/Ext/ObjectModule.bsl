...
Function ConvertCommand(Reader, Writer, VariableName, ObjectType = "")
	...
	ElsIf Reader.Name = "clickFormattedStringHyperlink" Then
		PresentationParam = Undefined;
		While Reader.ReadAttribute() Do
			If Reader.Name = "title" Then
				PresentationParam = Reader.Value;
			Else
				Raise NStr("en = 'Unknown attribute '; ru = 'Неопознанный атрибут '") + Reader.Name + ": " + Reader.Value;
			EndIf;
		EndDo;
		AddLine(Writer, VariableName + ? (ScriptVariant = "en", ".ClickFormattedStringHyperlink(""", ".НажатьНаГиперссылкуВФорматированнойСтроке(""") + DoubleQuotationMarks(PresentationParam) + """);", True);
		LastProcessedCommand = ? (ScriptVariant = "en", "ClickFormattedStringHyperlink", "НажатьНаГиперссылкуВФорматированнойСтроке");
		Reader.Read();
		
	ElsIf Reader.Name = "clickViewStatusItem" Then
		PresentationParam = Undefined;
		While Reader.ReadAttribute() Do
			...
			EndIf;
		EndDo;
		AddLine(Writer, VariableName + ? (ScriptVariant = "en", ".ClickViewStatusItem(""", ".НажатьНаЭлементСостоянияПросмотра(""") + DoubleQuotationMarks(PresentationParam) + """);", True);
		LastProcessedCommand = ? (ScriptVariant = "en", "ClickViewStatusItem", "НажатьНаЭлементСостоянияПросмотра");
		Reader.Read();
		
	ElsIf Reader.Name = "deleteViewStatusItem" Then
		PresentationParam = Undefined;
		While Reader.ReadAttribute() Do
			If Reader.Name = "item" Then
				PresentationParam = Reader.Value;
			...
			EndIf;
		EndDo;
		AddLine(Writer, VariableName + ? (ScriptVariant = "en", ".DeleteViewStatusItem(""", ".УдалитьЭлементСостоянияПросмотра(""") + DoubleQuotationMarks(PresentationParam) + """);", True);
		LastProcessedCommand = ? (ScriptVariant = "en", "DeleteViewStatusItem", "УдалитьЭлементСостоянияПросмотра");
		Reader.Read();
	...
	ElsIf Reader.Name = "goOneLevelUp" Then
		ConvertCommandPartGotoSpecificRow(Reader, VariableName, Writer, "GoOneLevelUp", "ПерейтиНаУровеньВверх");

	ElsIf Reader.Name = "goOneLevelDown" Then
		ConvertCommandPartGotoSpecificRow(Reader, VariableName, Writer, "GoOneLevelDown", "ПерейтиНаУровеньВниз");
		
	ElsIf Reader.Name = "isPossibleExpandNode" Then
		ConvertCommandPartGotoSpecificRow(Reader, VariableName, Writer, "CanBeExpanded", "ВозможноРазвернуть");
		
	ElsIf Reader.Name = "isCurrentRowNodeExpanded" Then
		ConvertCommandPartGotoSpecificRow(Reader, VariableName, Writer, "Expanded", "Развернут");
	...
	ElsIf Reader.Name = "expand" Then
		If ObjectType = "FormGroup" Then
			...
			Reader.Read();
		Else
			WithSubordinatesParam = False;
			While Reader.ReadAttribute() Do
				If Reader.Name = "withSubordinates" Then
					WithSubordinatesParam = Reader.Value;
				Else
					Raise NStr("en = 'Unknown attribute '; ru = 'Неопознанный атрибут '") + Reader.Name + ": " + Reader.Value;
				EndIf;
			EndDo;
			ConvertCommandPartRowDescription(Reader, VariableName, Writer, "Expand", "Развернуть", WithSubordinatesParam);
		Endif;
		LastProcessedCommand = ? (ScriptVariant = "en", "Expand", "Развернуть");
...
Procedure ConvertCommandPartRowDescription(Val Reader, Val VariableName, Val Writer, Val TermInt, Val TermRus, Val WithSubordinatesParam = Undefined)

	Reader.Read();
	SecondParam = "";
	If WithSubordinatesParam <> Undefined Then
		SecondParam = ", " + String(WithSubordinatesParam);
	EndIf;
	
	If Reader.NodeType = XMLNodeType.StartElement And Reader.Name = "Field" Then
		ConvertCommandPartRowDescriptionPartInt(Reader, Writer);
		AddLine(Writer, VariableName + ? (ScriptVariant = "en", "." + TermInt + "(RowDescription" + SecondParam + ");", "." + TermRus + "(ОписаниеСтроки" + SecondParam + ");"), True);
	Else
		AddLine(Writer, VariableName + ? (ScriptVariant = "en", "." + TermInt +"(" + SecondParam + ");", "." + TermRus + "(" + SecondParam + ");"), True);
	EndIf;
