		...
		Для Каждого ТекущийТест Из ДанныеТестирования.Алгоритмы цикл
			...
			Попытка
				Для СчетчикКоличество = 1 по ТекущийТест.Количество цикл
					...
					ТестированиеПриложенийСервер.ЗафиксироватьСобытиеТестирования(УИД, Лог, Ошибки);
					ВыполнитьТестовыйАлгоритм(ТестовоеПриложение, ТекущийТест.КодАлгоритма, Лог, СчетчикКоличество, Ошибки, УИД);
				КонецЦикла;
			Исключение
...
Процедура ВыполнитьТестовыйАлгоритм(ТестовоеПриложение, КодАлгоритма, Лог, Итерация, Ошибки, УИДЗадания) 
...
Процедура ЗапуститьВсеТесты() Экспорт 
	...
	Для Каждого УИД Из УИДыЗаданий цикл
		...
		ОбщегоНазначенияСервер.Пауза(5);
	КонецЦикла;	
